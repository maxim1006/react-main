...
nc (netcat) — название самой утилиты, которую часто называют «швейцарским армейским ножом» для работы с сетью.
...
Команда ping не может отправлять пакеты сразу на три адреса. Ей нужен только один конкретный IP.Поэтому операционная система берет первый попавшийся адрес из списка, который вернул DNS (в данном случае это 5.255.255.242), и начинает слать пакеты именно туда.

![ping_dig_nslookup.png](./img/ping_dig_nslookup.png)
