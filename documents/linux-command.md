# быстрая проверка доступности определенного сетевого порта
nc -vz google.com 443

nc -vv google.com 443 (если нужно с подробностями)

если зависает то не работает

nc (netcat) — nc использует TCP протокол и позволяет проверить TCP соединение, а название самой утилиты, которую часто называют «швейцарским армейским ножом» для работы с сетью.
-v (verbose) — подробный режим. Заставляет программу выводить текстовый результат проверки (успешно или нет). Без этого флага команда отработает молча.
-z (zero-I/O) — режим сканирования. Утилита просто пытается установить соединение (трехстороннее рукопожатие TCP) и сразу закрывает его, не передавая данные.
...
dig (и nslookup)
dig и nslookup опрашивают напрямую DNS-сервер. Эти утилиты игнорируют настройки вашего компьютера.
Команда dig спросила у DNS-сервера: «Какие IP-адреса есть у ya.ru?»DNS-сервер вернул список из трех доступных IP-адресов:5.255.255.24277.88.55.24277.88.44.242 Крупные сайты специально используют сразу несколько IP-адресов, чтобы распределять нагрузку между своими серверами.
Команда ping не может отправлять пакеты сразу на три адреса. Ей нужен только один конкретный IP. Поэтому операционная система берет первый попавшийся адрес из списка, который вернул DNS (в данном случае это 5.255.255.242), и начинает слать пакеты именно туда. Ping использует ICMP: Он отправляет эхо-запрос напрямую на сетевой уровень (Layer 3). Его единственный вопрос: «Этот IP-адрес вообще живой?» Ему не важны порты программ.

![ping_dig_nslookup.png](./img/ping_digns_nslookup.png)
...
traceroute: Warning: ya.ru has multiple addresses; using 77.88.55.242
traceroute to ya.ru (77.88.55.242), 64 hops max, 40 byte packets
1  192.168.68.1 (192.168.68.1)  5.715 ms  4.995 ms  4.363 ms
2  100.65.0.1 (100.65.0.1)  7.349 ms  6.828 ms  7.269 ms
